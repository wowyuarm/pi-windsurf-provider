...
export function buildGetChatMessageRequest(
  ...
  const parts: Uint8Array[] = [
    ...
    ...convertMessages(upstreamMessages, context.messages, conversationId).map((message) => encodeMessageField(3, message)),
...
function convertMessages(messages: Message[], allMessages: Message[], conversationId: string): Uint8Array[] {
  ...
  for (const message of messages) {
    ...
    if (message.role === "assistant") {
      ...
      const assistantMessageId = storedState?.messageId ?? message.responseId ?? stablePromptMessageId("assistant", conversationId, message, allMessages);
      const parts: Uint8Array[] = [
...
/**
 * Build the exact message history sent upstream.
 *
 * Windsurf's conversation_id is useful metadata, but it is not a reliable
 * model-context store across all model paths. To make this provider behave like
 * normal Pi providers, every request sends the current Pi branch history that the
 * model needs to see. Message ids are stable, so replayed messages remain the
...
export function getWindsurfDeltaMessages(messages: Message[], _conversationId: string): Message[] {
  return getWindsurfUpstreamMessages(messages);
}
